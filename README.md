...
`tools/list` at `https://gateway.pipeworx.io/disney/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/disney_search_characters \
  -H 'Content-Type: application/json' \
  -d '{"name":"Mickey Mouse"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/disney_search_characters`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
